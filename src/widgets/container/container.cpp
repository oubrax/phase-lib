...
void Container::adjust_fit(Widget& child) {
    if (along_axis(ContainerAxis::Row)) {
        ...
        if (style.h.unit == Unit::Fit || style.h.unit == Unit::Grow)
            layout.measured_h =
                std::max(child.layout.measured_h + height_additions(), layout.measured_h);
    } else {
        if (style.h.unit == Unit::Fit || style.h.unit == Unit::Grow)
        ...
        if (style.w.unit == Unit::Fit || style.w.unit == Unit::Grow)
            layout.measured_w =
                std::max(child.layout.measured_w + width_additions(), layout.measured_w);
    }
}
...
void Container::pct_check(Widget& child) {
    float base_w =
        compute_base(style.w.unit, style.w.value.px, layout.measured_w, style.pl, style.pr);

    float base_h =
        compute_base(style.h.unit, style.h.value.px, layout.measured_h, style.pt, style.pb);
...
static float compute_cross_space(float container_size, float pad_a, float pad_b, float child_size) {
    return container_size - pad_a - pad_b - child_size;
}

static float align_offset(ContainerAlign align, float space) {
    switch (align) {
    case ContainerAlign::Start:
        return 0.f;
